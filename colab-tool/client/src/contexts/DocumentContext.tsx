...
export const DocumentProvider: React.FC<DocumentProviderProps> = ({ children }) => {
  ...
  const createDocument = async (title: string): Promise<string | null> => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/documents', { title });
  ...
  const updateDocumentTitle = async (id: string, title: string) => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/documents/${id}/title`, { title });
  ...
  const addCollaborator = async (documentId: string, email: string) => {
    try {
      ...
      await axios.post(`http://localhost:5000/api/documents/${documentId}/collaborators`, { email });
  ...
  const removeCollaborator = async (documentId: string, collaboratorId: string) => {
    try {
      ...
      await axios.delete(`http://localhost:5000/api/documents/${documentId}/collaborators/${collaboratorId}`);
